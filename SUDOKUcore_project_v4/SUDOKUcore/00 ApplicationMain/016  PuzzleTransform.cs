...
namespace GNPXcore{
    public class PuzzleTrans{
        ...
        private GNPX_App pGNP;
        private GNPZ_Engin pGNPX_Eng{ get=> pGNP.pGNPX_Eng; }
        private UPuzzle    pPZL{ get=>pGNPX_Eng.PZLMan.PZL; set=>pGNPX_Eng.PZLMan.PZL=value; }
        public int         ID{ get=>pPZL.ID; }
      ...
      //----------------------------
        ...
        public PuzzleTrans( GNPX_App GNP ){
            this.pGNP=GNP;
        }
        ...
        private void Initialize( bool StartF=true ){          
            ...
            if(StartF) PZLorg = pPZL.Copy( );
            PZLbas = pPZL.Copy( );
        }

        public void btnTransEst(){
            if(pPZL.AnsNum==null) return;
        ...
        public void SDK_TransProbG( string ctrl, bool DspSolB ){
            ...
            UPuzzle aPZL=pPZL.Copy( );
            aPZL.BOARD=UCL; aPZL.AnsNum=AnsN2;
            if(ID>=0) pGNP.SDK_PUZZLE_List[ID] = aPZL;
            else{
                aPZL.ID = pPZL.ID = 0;
                pGNP.SDK_PUZZLE_List.Add(aPZL);
            }
            if(!DspSolB) aPZL.BOARD.ForEach(P=>{P.No=Max(P.No,0);});
            pGNP.current_Puzzle_No=ID;
        ...
        public string SDK_Nomalize( bool DspSolB, bool NrmlNum ){
            ...
            if(pPZL.AnsNum==null) Initialize(); //Solve
       ...
       //===== step3 =====
            ...
            for(int k=0; k<TPLst.Count; k++ ){
                ...
                string st=pGNP.SDKCntrl.Get_SDKNumPattern(TrPara,pPZL.AnsNum);//Latin square
            ...
            if(NrmlNum){
                ...
                for(int rc=0; rc<81; rc++ ){
                    int No=pPZL.AnsNum[rc];
                    UCell P=pPZL.BOARD[rc];
                    if(P.No>0) pPZL.AnsNum[rc]=P.No=chgNum[No];
                    else       pPZL.AnsNum[rc]=P.No=-chgNum[-No];
                }
            }
                /*
                            string st="◆";
