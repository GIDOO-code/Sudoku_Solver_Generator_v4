...
namespace GNPXcore{   
    ...
    static public class StaticSA{ 
      ...
      #region ToString_SameHouseComp
        ...
        static public string ToBitString( this int num, int ncc=9 ){
            int numW = num;
            string st="";
            for(int k=0; k<ncc; k++ ){
    ...
    #region Enumerators
        static public IEnumerable<UCell> IEGetCellInHouse( this List<UCell> pBDL, int h, int FreeB=0x1FF ){
        ...
        static public void IE_SetNoBBgColor( this Bit81 BX, List<UCell> pBDL, int noBX, SWMColor AttCr, SWMColor Cr ){ 
            foreach( var p in BX.IEGetUCell_noB(pBDL,noBX) ) pBDL[p.rc].Set_CellColorBkgColor_noBit(noBX,AttCr,Cr); 
        }
        static public void IE_SetNoBBgColor( this List<UCell> LstUCell, List<UCell> pBDL, int noBX, SWMColor AttCr, SWMColor Cr ){ 
            LstUCell.ForEach( p => pBDL[p.rc].Set_CellColorBkgColor_noBit( noBX, AttCr, Cr) );
        }
        static public IEnumerable<UCell> IEGetUCell_noB( this Bit81 BX, List<UCell> pBDL, int noBX ){ //nx=0...8        
            for(int n=0; n<3; n++ ){
                int bp = BX._BP[n];
                for(int k=0; k<27; k++ ){
