...
namespace GNPXcore{
    ...
    // Functions are bit operation(arithmetic, logical, string functions).
        ...
        public Bit81( bool all1 ): this(){
            if(all1) this._BP[0]=this._BP[1]=this._BP[2]= 0x7FFFFFF;
        }

        public int Get_rcbBitRep( Bit81 rcB, int ID ){
            this.ID=ID;
            int rcbB=0;
...
//###        static private int[] __RCBpattern;  // rc -> bit expression of Row/Column/Block
        ...
        static Bit981(){
            ...
            for(int rc=0; rc<81; rc++){
                __RCBpattern[rc] = rc.ToRCB_BitRep();
        ...
        public Bit981( List<UCell> pBDL, bool eSet=true ){
            _BQ = new Bit81[9];
            for( int no=0; no<9; no++ ){ 
                int noB = (1<<no);
                Bit81 BD = new Bit81(pBDL,noB);   
