...
namespace GNPXcore{
    public class UGrCells: List<UCell>, IComparable, IEquatable<UGrCells> {
      //Related cell group in house
        public readonly int  h;  //house No.
        ...
        public UGrCells(int h, int no){
            this.h=h; this.no=no;
            B81=new Bit81();
        }
        public UGrCells(int h, int no, UCell Q): this(h,no){ Add(Q); }
        public UGrCells Copy(){
            UGrCells Pcpy=new UGrCells(h,no);
        ...
        public bool Equals(UGrCells other){
            return other != null &&
                 //h == other.h &&      //Difference in h is acceptable. Important as an algorithm.
                   no == other.no &&
        ...
        public override int GetHashCode(){
          ...
          //hashCode = hashCode * -1521134295 + h.GetHashCode();  //Difference in h is acceptable. Important as an algorithm.
