...
namespace GNPXcore {
    public partial class SuperLinkMan{
        ...
        public USuperLink get_L2SprLKEx( int rc0, int no0, bool FullSearchB, bool DevelopB=false ){
            USuperLink USLK=null;
            if( FullSearchB || !L2SprLkB81[no0].IsHit(rc0) ){                       // Search when Unconditional or Unprocessed    
                USLK = Eval_SuperLinkChainEx( rc0, no0, DevelopB:DevelopB );        // Stem : rc0 #no0       
        ...
        public USuperLink Eval_SuperLinkChainEx( int rc0, int no0, bool DevelopB ){
			try{	
                ...
                {//====================== Start with WeakLink(origin:rc#no0) ======================
                    ...
                    foreach( var nox in pBOARD[rc0].FreeB.IEGet_BtoNo().Where(p=>p!=no0)){
				...
				while(rcQue.Count>0){ 
					...
					foreach(var RR in IEGet_SuperLink(R).Where(p=>p.AvailableF) ){      //foreach next GLink 
                        ...
                        if( (R.UsedCs&RR.UGCellsB.B81).IsNotZero() )  continue;            //Skip by used Cells
		...
		public  void developDispEx( int rc0, int no0, USuperLink USLK, bool DevelopB ){ //Color the cells. Generate link string.
            ...
            foreach( var P in qBDL ){
                ...
                if(noT>0) P.Set_CellColorBkgColor_noBit(     noT, Colors.Red, Colors.LemonChiffon  );
                if(noF>0) P.Set_CellDigitsBkgColorRev_noBit(noF, Colors.Red, Colors.LemonChiffon  );
                if((noT&noF)>0){
                    P.Set_CellColorBkgColor_noBit(noT&noF, Colors.White , Colors.PowderBlue );
                    P.Set_CellDigitsColorRev_noBit(noT&noF,Colors.Blue );
                }
            }
            qBDL[rc0].Set_CellColorBkgColor_noBit(1<<no0, Colors.Red, Colors.Yellow);
