    ...
    public partial class SuperLinkMan{     
			...
			try{	
				...
				while(rcQue.Count>0){
					...
					foreach( GroupedLink RR in IEGet_SuperLink(R).Where(p=>p.AvailableF) ){                                               
                        ...
                        if( (R.UsedCs&RR.UGCellsB.B81).IsNotZero() )  continue;
						...
						if(RR.type==S){   //========== Check connection conditions. case StrongLink(F->T) ==========
					      //if( (RR.UGCellsB.B81&Qtrue[RR.no2]).IsNotZero() )  continue;   //xxx if the next element is already setted true, go to the next process
							if(RR.UGCellsB.Count==1){                                       //if the link's next element is a single cell
							...
							if( (RR.UGCellsB.B81&Qfalse[RR.no2]).IsNotZero() )  continue;      //if the next element is setted false, go to the next process
        ...
        private int GroupedNLEx_CheckSolution( USuperLink GNL_Result, ref int contDiscontF){ 
            ...
            if(SolType){        //==================== continuous ====================
                ...
                UCell P=pBOARD[GLKorg.UGCellsA[0].rc];   //(for MultiAns code)
                switch(dcTyp){
                    case 11: 
