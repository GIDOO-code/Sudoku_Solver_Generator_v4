...
namespace GNPXcore {
    public partial class SuperLinkMan{
        ...
        private List<UCell>    pBOARD{ get{ return pAnMan.pBOARD; } }
...
//		private bool DevelopB=false;  //true; false;
        ...
        public void PrepareSuperLinkMan( bool AllF=false ){
            ...
            if( AllF || (bool)GNPX_App.GMthdOption["GroupedCells"] )  GLKMan.Initialize();
            if( AllF || (bool)GNPX_App.GMthdOption["ALS"] )           ALSMan.QSearch_AlsInnerLink();
        ...
        public USuperLink Eval_SuperLinkChain( int rc0, int no0, bool Gbreak, int typeSel, bool DevelopB ){
			...
			try{	
				...
				UCell P0=pBOARD[rc0];     //Origin Cell
                ...
                if( (typeSel&2)>0 ){//====================== Start with WeakLink(origin:rc#no0) ======================
				    foreach( var GLKH in IEGet_SuperLinkFirst(P0,no0).Where(X=>X.type==W) ){	
					    ...
					    foreach( var P in GLKH.UGCellsB.Where(p=>(p.FreeB&no0B)>0) ){
							...
							if( Gbreak && Qtrue[no2].IsHit(P.rc) && GenMessage2(USLK,pBOARD[P.rc],no2) ) goto FTBreak; //found error
							...
							if(P.FreeBC==2){                                        //If the next is a binary cell
								...
								if( Gbreak && Qfalse[nox].IsHit(P.rc) && GenMessage2(USLK,pBOARD[P.rc],nox) ) goto FTBreak; //found error
							}
					    }
				    }
                ...
                if( (typeSel&1)>0 ){//====================== Start with StrongLink(origin:rc#no0) ======================
				    foreach( var GLKH in IEGet_SuperLinkFirst(P0,no0).Where(X=>X.type==S) ){	
					    ...
					    if( GLKH.UGCellsB.Count==1 ){                               //if the link's next element is a single cell
						    ...
						    if( Gbreak && Qfalse[no2].IsHit(P.rc) && GenMessage2(USLK,pBOARD[P.rc],no2) ) goto FTBreak; //found error       
						    ...
						    if( Gbreak && Qtrue[nox].IsHit(P.rc) && GenMessage2(USLK,pBOARD[P.rc],nox) ) goto FTBreak; //found error
					    }
				    }
                }
            #endregion stsrt
                ...
                while(rcQue.Count>0){   
                    ...
                    foreach( var GLKH in IEGet_SuperLink(R) ){                                  //foreach next GLink                            
						...
						if(GLKH.type==S){                                                   //Check connection conditions. case StrongLink
							if( (GLKH.UGCellsB.B81&Qtrue[GLKH.no2]).IsNotZero() )  continue;  //if the next element is setted true, go to the next process
							if( GLKH.UGCellsB.Count==1 ){                                     //if the link's next element is a single cell
								var P=GLKH.UGCellsB[0];                                       //P:next cell
								Qtrue[no2].BPSet(P.rc);                                       //set P.rc#no2 is true
								chainDesLKT[P.rc,no2]=GLKH;                                   //record in retroactive chain
						...
						if( GLKH.type==W ){                                                 //Check connection conditions. case WeakLink
							if( (GLKH.UGCellsB.B81&Qfalse[GLKH.no2]).IsNotZero() )  continue;  //if the next element is setted false, go to the next process
        ...
        public IEnumerable<GroupedLink> IEGet_SuperLinkFirst( UCell UC, int no ){
            ...
            if( (bool)GNPX_App.GMthdOption["GroupedCells"] ){
        ...
        public IEnumerable<GroupedLink> IEGet_SuperLink( GroupedLink GLKpre ){
            ...
            if( (bool)GNPX_App.GMthdOption["ALS"] && ALSMan.AlsInnerLink!=null){
                if( GLKpre.type==W ){
            ...
            //      ALK.UGCellsB.ForEach(P=>{ if((P.FreeB&noB)>0) BP.BPSet(P.rc); });
                ...
                for(int h=0; h<27; h++ ){
                    Bit81 HS = BPnoB&pHouseCells[h];
                    if( (BP-HS).IsNotZero())  continue;
...
//C                        WriteLine( "NxtBP:"+NxtBP );
                    ...
                    List<UCell> NxtCs= NxtBP.ToList().ConvertAll(rc=>pBOARD[rc]);
                    for(int k=1; k<(1<<NxtCs.Count); k++ ){
                        UGrCells NxtGrpdCs=new UGrCells(h,ALK.no2);
                        int kb=k;
                        for(int n=0; n<NxtCs.Count; n++){
                            if( (kb&1)>0 )  NxtGrpdCs.Add( new UGrCells(NxtCs[n],ALK.no2) );
                            kb>>=1;
                        }
                        GroupedLink GP = new GroupedLink(GLKpre.UGCellsB,NxtGrpdCs,h,W);
//C                        WriteLine( GP );
                        yield return GP; 
                    }
        ...
        public bool Check_SuperLinkSequence( GroupedLink GLKpre, GroupedLink GLKnxt ){
            ...
            int FreeBC=0;
            if( LKpre!=null ){ 
                FreeBC = pBOARD[LKpre.rc2].FreeBC;
...
//============================================================================================================
        ...
        public string GenMessage2FakeProposition(int rc0, int no, USuperLink USLK, UCell Q ){
            string _krfMsgEx0 = $"{rc0.ToRCString()}#{no+1} is false(contradition)";
            string st = $"ForceChain_Cell {_krfMsgEx0}";
		...
		public string _chainToString2( UCell U, GroupedLink Gdes, int noRem ){
            ...
            try{     
                ...
                var X=Gdes;
                while( !(X.h==-1 && X.no==X.no2 && X.rootF) ){
                    Qlst.Add(X);
