...
namespace GNPXcore{
    public partial class UGLinkMan2{
        ...
        public int PrepareUGLinkMan( bool printB=false ){                   //Initialization for stage
            ...
            for(int no=0; no<9; no++)  _noPat[no] = new Bit81(pBOARD,(1<<no)); //Bit expression of cell with number n
...
/*####*/                if (++kcc>=kccLim){ UGLCovCandidates.Add(P); break; }//number of digits contained in BaseSet satisfies the condition(kccLim). /*####*/
      ...
      #region link_search
        private void _LinkSearch( bool printSW=false ){
            ...
            for(int no=0; no<9; no++){
                for(int h=0; h<27; h++ ){
                    ...
                    int SW = (szL==2)? 0: 1;
                    IDx++;
                    var LK1 =new UGLink_unit(IDx,SW,h,no,PLst);
                    GLK_UList_All.Add(LK1);
                    Combination cmb=new Combination(szL,2);
                    while( cmb.Successor() ){
                        UCell UC1=PLst[cmb.Index[0]], UC2=PLst[cmb.Index[1]];
                        __SetGLinkLstSet(LK1,SW,h,no,UC1,UC2);
                    }
                }
            }

            // *==* in-cell link
            foreach( var P in pBOARD.Where(p => p.No==0) ){
                int rc=P.rc;
        ...
        private void __SetGLinkLstSet(UGLink_unit LKx, int SW, int h, int no, UCell UC1, UCell UC2 ){     // cell-cell link
            ...
            public void Prepare( Bit324 usedLKIDsq, List<UGLink_unit> BaseSetLst, Bit981 HB981,  int[] RCB_frameB){ 
                ...
                foreach( var UGL in BaseSetLst){
                    if(UGL.type==0){        //--- type:link ---
                        ...
                        int rc=UGL.rc;
                        foreach(var no in UGL.FreeB.IEGet_BtoNo()) rcbnFrame9[no] |= (long) rc.ToRCB_BitRep();
                    }
                }
            }
                ...
                { //There is a limit to the number of cells that have no links other than BaseSet.                   
                    foreach(var UGL in BaseSetLst){
                        if(UGL.type==0){
                            ...
                            //if(SDK_Ctrl.UGPMan.stageNo==10) WriteLine($"+++ h:{h} RCB_frameB[{no}]:{RCB_frameB[no].ToBitString27()}");
                        }
                    }
                }
