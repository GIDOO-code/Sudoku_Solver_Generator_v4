            ...
            for(int no=0; no<9; no++ ){  //#no
                ...
                for( int hs=0; hs<18; hs++ ){   //   0-8:row 9-17:column
                    ...
                    { //----- found -----
                        ...
                        Result=SolMsg; 
                        if( __SimpleAnalyzerB__ )  return true;
                        foreach( var P in pBDL.IEGetCellInHouse(18+b1,noB)) P.Set_CellColorBkgColor_noBit( noB, AttCr3, SolBkCr );
                        foreach( var P in pBDL.IEGetCellInHouse(18+b2,noB)) P.Set_CellColorBkgColor_noBit( noB, AttCr3, SolBkCr );
                        if( SolInfoB ) ResultLong=SolMsg;
                        if( !pAnMan.SnapSaveGP(pGP) )  return true;
                    }
                }
