...
namespace GNPXcore{
    ...
    public partial class AALSTechGen: AnalyzerBaseV2{
        ...
        public IEnumerable<UCell> _GetRestCells( UALS ISP, int selB ){
            return pBDL.IEGetCellInHouse(ISP.h,selB).Where(P=>!ISP.B81.IsHit(P.rc));
        }
        private void SuDoQueEx_SolResult( UALS ISPB, UALS ISPR ){
            ...
            if( SolInfoB ){
                ISPB.UCellLst.IE_SetNoBBgColor( pBDL, 0x1FF, AttCr, SolBkCr );    // FreeB_Clr, Digit_Clr, background_Clr
                ISPR.UCellLst.IE_SetNoBBgColor( pBDL, 0x1FF, AttCr, SolBkCr );
