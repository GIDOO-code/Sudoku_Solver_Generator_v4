    ...
    public partial class ALSTechGen: AnalyzerBaseV2{
		...
		private void Prepare(){
			if( stageNo != stageNoMemo ){
                ...
                BP981 = new Bit981(pBDL);
                BD0 = new Bit81(pBDL,0x1FF);
			}      
		}

        //ALS XY-Wing is an analysis algorithm using three ALS. It is the case of the next ALS Chain 3ALS.
        // https://gidoo-code.github.io/Sudoku_Solver_Generator/page44.html
        ...
        // https://gidoo-code.github.io/Sudoku_Solver_Generator/page44.html   - XYZ-WingALS
        // https://gidoo-code.github.io/Sudoku_Solver_Generator/page51.html   - ALS XY-Wing
#endif
        ...
        private    bool _XYZwingALSSub( int wsz ){ //simple UVWXYZwing
            ...
            foreach( var P0 in FBCX ){              // Stem cell
                ...
                for( int no=0; no<9; no++){ //No relation between P0.FreeB and excluded digit(no).
                    ...
                    Bit81 B81_P0_conn = (new Bit81(pBDL,noB)) & ConnectedCells[P0.rc];  //cells related to rc/no
                            ...
                            foreach( var ALSin in ALSMan.IEGetCellInHouse(1,noB,B81_P0_block2,18+b0) ){ //ALS in Stem b0
                                ...
                                if(SolFound){
                                    ...
                                    if( SolInfoB ){
                                        ...
                                        ALSin.UCellLst.IE_SetNoBBgColor(  pBDL, 0x1FF, AttCr3, SolBkCr);
                                        ALSout.UCellLst.IE_SetNoBBgColor( pBDL, 0x1FF, AttCr3, SolBkCr);
                                        ...
                                        string msg3 = $" Eliminated: {pBDL.FindAll(p=>p.CancelB>0).ToRCString()} #{no+1}";   
                                    ...
                                    if( !pAnMan.SnapSaveGP(pGP) ){ break_XYZwingALS=true; return true; }
                                    SolFound=false;
                                }
                            }
                        }
