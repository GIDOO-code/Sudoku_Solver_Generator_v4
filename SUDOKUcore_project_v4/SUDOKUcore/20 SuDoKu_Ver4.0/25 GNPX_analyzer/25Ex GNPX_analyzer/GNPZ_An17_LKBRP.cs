        ...
        public bool RemotePair( ){     //RemotePairs
            ...
            foreach( var (CRL,FreeB) in _RPColoring()){
                ...
                if( RPFound ){ //=== found ===
                    ...
                    foreach(var P in CRL[0].IEGet_rc().Select(p=>pBDL[p]))  P.Set_CellColorBkgColor_noBit( FreeB, AttCr, Cr1 );
                    foreach(var P in CRL[1].IEGet_rc().Select(p=>pBDL[p]))  P.Set_CellColorBkgColor_noBit( FreeB, AttCr, Cr2 );
