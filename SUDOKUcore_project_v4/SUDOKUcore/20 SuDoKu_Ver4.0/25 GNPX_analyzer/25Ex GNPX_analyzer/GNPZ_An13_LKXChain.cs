...
namespace GNPXcore{
    public partial class NXGCellLinkGen: AnalyzerBaseV2{
        ...
        Color Cr = _ColorsLst[0];
            ...
            for(int no=0; no<9; no++ ){
                ...
                foreach( var (CRL,rcS) in _GetXChain(no) ){
                    ...
                    // ===== X-Chain found =====
                    Bit81 ELM = CRL[2];
                    foreach( var P in ELM.IEGetUCell_noB(pBDL,noB) )  P.CancelB=noB; 
                    SolCode = 2;   
                    string SolMsg=$"X-Chain #{(no+1)}  rcS:{rcS.ToRCString()}({rcS})";
                    Result=SolMsg;
                    if( SolInfoB ){  
                        (CRL[0]-ELM).IE_SetNoBBgColor( pBDL, noB, AttCr, Cr1 );     
                        (CRL[1]-ELM).IE_SetNoBBgColor( pBDL, noB, AttCr, Cr2 );  
                        ELM.IE_SetNoBBgColor( pBDL, noB, AttCr, Cre );
                        ResultLong=SolMsg;
                    }
        ...
        private IEnumerable<(Bit81[],int)> _GetXChain( int no ){    //## Repeatedly coloring processing.
            Bit81 BPno = new Bit81( pBDL,(1<<no) );                 // Bit representation of the cells with #no as a candidate
