...
namespace GNPXcore{
    ...
    public class UGLink_unit{
        ...
        public long    RCBN_frameB_Remove_tfx;  //Remove h from RCBN_frameB
        ...
        public  int  h{  set=> UGL|=(value<<4); get=>(UGL>>4)&0x1F; }
                
        public  int  no{ set=> UGL|=value; get=>(UGL&0xF); }
        public  int  rc{ set=> UGL|=(value<<4); get=>(UGL>>4)&0xFF; } 
        public  UGLink_unit(){ }
        public  UGLink_unit( int IDx, int SW, int  h, int no, List<UCell> PLst ){    //link
            this.IDsq=IDx; this.SW=(SW&1); this.type=0; this.h=h; this.no=no;
            var _conn=new Bit81();
            ...
            RCBN_frameB_Remove_tfx = RCBN_frameB_Remove_tfx.DifSet(h);     //Remove h from RCBN_frameB
        ...
        public  UGLink_unit( int IDx, int SW, int rc, int FreeB ){                     //Cell
            ...
            RCBN_frameB_Remove_tfx  = (long)_rcbFrame | (long)FreeB<<27;   //Remove h from RCBN_frameB
        ...
        public override string ToString(){
            ...
            string st2="";
            if(type==0){ st += $"h:{h} no:{no+1} <link>"; st2+=$" {h.hToString()}#{no+1}"; }       //0:link
        ...
        public string ToAppearance( bool Gen=false){
            string st;
            if(type==0){ st = $"{h.hToString()}#{no+1}"; }       //0:link
        ...
        public bool existAnotherLink( int rcX ){
            ...
            var B81P0Hdex = rcnElementLst.FindIndex(P=>((P>>4)==rcX));
            if(B81P0Hdex<0) return false;
