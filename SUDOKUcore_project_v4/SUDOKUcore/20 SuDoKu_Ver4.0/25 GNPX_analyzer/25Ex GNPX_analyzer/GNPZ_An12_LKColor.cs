...
namespace GNPXcore{
    public partial class NXGCellLinkGen: AnalyzerBaseV2{
        ...
        public bool Color_Trap( ){
            ...
            //4....9.5.23..58.67...4.7.........3253.2....8.5.1...7.....89....9......7..1.72..46  by /Solve/MultiSolve step19 sq.19
            ...
            for(int no=0; no<9; no++ ){
                ...
                foreach( Bit81[] CRL in _Coloring(no) ){                //_Coloring: cells_network connected by strong-links
                    ...
                    HitB.Clear();
                    Bit81 ELM = (new Bit81(pBDL,noB))-(CRL[0]|CRL[1]);  //ELM:Exclude cells from CRL_cells with digit(no)
                    ...
                    if( HitB.IsNotZero() ){ //===== found =====
                        ...
                        if( SolInfoB ){
                            ...
                            CRL[0].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr );
                            CRL[1].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr1 );
                        }
        ...
        public bool Color_Wrap( ){
            //Paste the next 81 digits onto the grid and solve with /Solve/MultiSolve/    
            //..9..154..5..9.....6.5..92.8..3..1.41..4.6..95.4..8..2.42..9.6.....6..1..187..2..  by /Solve/MultiSolve step3 sq.22
			Prepare();
			CeLKMan.PrepareCellLink(1);                                 //Generate StrongLink
                ...
                foreach( Bit81[] CRL in _Coloring(no) ){                // _Coloring: cells_network connected by strong-links
                    ...
                    for(int k=0; k<2; k++ ){
                        ...
                        for(int h=0; h<27; h++ ){
                            ...
                            //===== found =====
                            foreach( var Q in CRL[k].IEGet_rc().Select(p=>pBDL[p]) ) Q.CancelB=noB; 
                            ...
                            if( SolInfoB ){
                                ...
                                CRL[1-k].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr1 );
                                CRL[k].IE_SetNoBBgColor(   pBDL, noB, AttCr, Cr2 );
                            }
                            if( __SimpleAnalyzerB__ )  return true;
                            if( !pAnMan.SnapSaveGP(pGP) )  return true;
                        }
        ...
        public bool MultiColor_Type1( ){
            //Paste the next 81 digits onto the grid and solve with /Solve/MultiSolve/
            //..9..154..5..9.....6.5..92.8..3..1.41..4.6..95.4..8..2.42..9.6.....6..1..187..2..  by /Solve/MultiSolve step8 sq.12
                    ...
                    for(int na=0; na<2; na++ ){
                        ...
                        for(int nb=0; nb<2; nb++ ){
                            ...
                            foreach( var rc in ELMtry.IEGet_rc() ){
                                ...
                                pBDL[rc].CancelB=noB; ELM.BPSet(rc); found=true;
                            }
                            if(found){
                                SolCode = 2;
                                ...
                                if( SolInfoB ){
                                    ...
                                    foreach( var P in ELM.IEGet_rc().Select(p=>pBDL[p]) ) P.Set_CellColorBkgColor_noBit(noB,AttCr,CrA);
                                    string st1 = "";
                                    for(int k=0; k<2; k++){
                                        Bit81[] CRLX = MCRL[cmb.Index[k]];
                                        Color Cr1=_ColorsLst[k*2];
                                        Color Cr2=Cr1; Cr2.A=(byte)100;          
                                        CRLX[1-k].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr1 );
                                        CRLX[k].IE_SetNoBBgColor(   pBDL, noB, AttCr, Cr2 );
        ...
        public bool MultiColor_Type2( ){
            //Paste the next 81 digits onto the grid and solve with /Solve/MultiSolve/
            //...6.8...6...9...529.....483.1...4.64..3.1..2...8.6....1.4.2.7..6.7.9.5.....8....  by /Solve/MultiSolve step11 sq.49-51
            ...
            for(int no=0; no<9; no++ ){
                ...
                while(prm.Successor()){
                    ...
                    for(int nb=0; nb<2; nb++ ){
                        ...
                        //===== found =====
                        SolCode = 2;
                        foreach( var P in CRLb[nb].IEGet_rc().Select(p=>pBDL[p]) ) P.CancelB=noB;                       
                        ...
                        if( SolInfoB ){
                            ...
                            CRLa[0].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr1 );
                            CRLa[1].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr2 );

                            Cr1 = _ColorsLst[1];   
                            Cr2 = Color.FromArgb( 100, Cr1.R, Cr1.G, Cr1.B );
                            CRLb[1-nb].IE_SetNoBBgColor( pBDL, noB, AttCr, Cr2 );
                            CRLb[nb].IE_SetNoBBgColor(   pBDL, noB, AttCr, Cr1 );
...
#region Abstraction code version
        ...
        private IEnumerable<Bit81[]> _Coloring( int no, int minSize=1, bool bMulti=false ){ 
            ColoringQueue QX = new ColoringQueue(pBDL,no,bMulti);
        ...
        private class ColoringQueue{
            ...
            public ColoringQueue( List<UCell> pBDL, int no, bool bMulti=false  ){
                this.Que  = new Queue<int>();
                this.BPno = new Bit81(pBDL,(1<<no));
                this.bMulti = bMulti;
...
#region Naive code version
        private IEnumerable<Bit81[]> _Coloring_Basic( int no ){
            ...
            Bit81 BPno = new Bit81(pBDL,(1<<no));
            int  rc1;
